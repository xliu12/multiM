clustered <- function(data, 
    ...
    data_in <- data %>% mutate(
        ...
        AMint = data_in$A * data_in[, Mnames[1]] * data_in[, Mnames[2]]
    ) 
    ...
    folds <- make_folds(cluster_ids = data[[Sname]], # Clusters are treated as a unit – that is, all observations within a cluster are placed in either the training or validation set.

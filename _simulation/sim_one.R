...
GenData <- function(
    ...
    if (quadratic.A == TRUE) {
        ...
        # Xquad <- model.matrix(~ -1 + .^2, data = data.frame(x))
        Xquad <- Xquad * data$Z
    ...
    if (quadratic.M == TRUE) {
        ...
        # Xquad <- model.matrix(~ -1 + .^2, data = data.frame(data$X))
        Xquad <- Xquad * data$Z
...
condition_all <- data.frame(expand.grid(
    ...
    icc = c(0.2, 0.4),
    x_z = c(0, 0.3),
    Yfamily = c("gaussian", "binomial") # c("gaussian"),, "binomial"
...
condition <- condition_all %>%
    filter(
        # Fit == "mlr",
        quadratic.A + quadratic.M + quadratic.Y == 3
        ) %>%
    filter(
...
methds <- methds_all %>% 
    filter(cluster_a == "FE" & cluster_m == "FE", cluster_y == "FE") %>%
    mutate(
...
OneData <- function(iseed = 1, cond = 1){
    ...
    one.jj <- function(jj = 1) {
        ...
        if (Fit == "mlr") {
            learners_a <- learners_m <- learners_y <- c("SL.ranger", "SL.earth") #c("SL.ranger", "SL.glm"), "SL.lightgbm"
            num_folds <- 1
        }
    ...
    res <- data.frame(condition[cond, ],
                      ...
                      # true_val = true_effs, # already included in one_boot
                      one_boot, 
                      row.names = NULL)
